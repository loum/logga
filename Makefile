PY=/usr/bin/python
NOSE=/usr/bin/nosetests1.1 -s -v --with-xunit --with-coverage --cover-erase --cover-package logga
NOSE_ENV=.env/bin/nosetests -s -v --with-xunit --with-coverage --cover-erase --cover-package logga
GIT=/usr/bin/git
...
install:
	$(RPM) -ivh dist/python-logga-?.??-?.noarch.rpm

upgrade:
	$(RPM) -Uvh dist/python-logga-?.??-?.noarch.rpm
